  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <main role="main" aria-label="Careers page">
        ...
        <section className="py-16 md:py-24 bg-[#4F2D7F]" aria-labelledby="opportunities-heading">
          <div className="max-w-6xl mx-auto px-4 md:px-8">
            <ScrollReveal>
              <div className="flex flex-col md:flex-row md:items-end md:justify-between mb-12">
                <div>
                  <span className="label-text text-[#CF2020] mb-2 block">CURRENT OPENINGS</span>
                  <h2 id="opportunities-heading" className="heading-xl text-white mb-4">
                    Open Positions
                  </h2>
                  <p className="body-lg text-white/80 max-w-xl">
                    Explore our current opportunities and find the role that matches your skills and ambitions.
                  </p>
                </div>
                <Link
                  href="/careers/opportunities"
                  className="mt-6 md:mt-0 inline-flex items-center text-white font-medium hover:text-[#CF2020] transition-colors"
                >
                  View all positions
            ...
            <StaggerContainer className="grid gap-4" staggerDelay={0.1}>
              {jobOpenings.map((job, index) => (
                <StaggerItem key={index}>
                  <Link 
                    href={`/careers/opportunities/${job.title.toLowerCase().replace(/\s+/g, '-')}`}
                    className="block bg-white/10 backdrop-blur-sm rounded-xl p-6 hover:bg-white/20 transition-all group"
                  >
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                      <div>
                        <h3 className="heading-sm text-white mb-2 group-hover:text-[#CF2020] transition-colors">
                          {job.title}
                        </h3>
