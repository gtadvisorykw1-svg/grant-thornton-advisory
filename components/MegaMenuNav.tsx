        ...
        {Object.entries(navigationData).map(([key, data]) => (
          ...
          >
            {data.hasDropdown ? (
              ...
              >
                ...
                <span className={`absolute right-[-12px] top-0 bottom-1 flex items-center transition-transform duration-300 ${activeMenu === key ? 'rotate-180' : ''}`}>
                  <DropdownArrow 
                    ...
                    className={activeMenu === key ? 'text-gt-coral' : 'text-[#A06DFF]'} 
                  />
                </span>
                {/* Animated underline */}
                <span 
                  className={`absolute bottom-0 left-0 h-[2px] bg-gt-coral transition-all duration-350 ${
                    activeMenu === key ? 'w-full' : 'w-0'
                  }`}
                  style={{
                    backgroundImage: 'linear-gradient(var(--color-gt-coral), var(--color-gt-coral))',
              ...
              <Link
                href={data.href}
                className="mega-nav-link relative text-base leading-[18px] whitespace-nowrap text-gt-purple hover:text-gt-coral pb-1 transition-all duration-300"
              >
                {data.label}
                <span className="absolute bottom-0 left-0 w-0 h-[2px] bg-gt-coral hover:w-full transition-all duration-350" />
              </Link>
            )}
                  ...
                  <div className="max-w-[1268px] mx-auto px-8 py-4 pb-20">
                    ...
                    {'groupTitle' in data && data.groupTitle && (
                      <div className="mt-6 mb-2 text-[#666666] font-light">
                        <Link href={data.href} className="text-gt-purple-dark hover:text-gt-purple transition-colors">
                          {data.groupTitle}
                        </Link>
                      </div>
                    )}
                        ...
                        >
                          ...
                          {'hasExpander' in column && column.hasExpander ? (
                            <div className="inline-flex items-center">
                              <Link 
                                href={column.href}
                                className="flex items-center text-gt-purple-dark font-light hover:text-gt-purple transition-colors"
                              >
                            ...
                            <Link 
                              href={column.href}
                              className="inline-flex items-center text-gt-purple-dark font-light hover:text-gt-purple transition-colors"
                            >
                              {column.title}
                            </Link>
                          )}
                              ...
                              <ul className="border-l border-[#A06DFF] pl-4 pt-1">
                                {column.links.map((link, linkIdx) => (
                                  ...
                                  >
                                    <Link
                                      href={link.href}
                                      className="inline-flex items-center text-sm leading-[22px] text-gt-purple hover:text-gt-coral transition-colors"
                                    >
                                      {link.label}
                                    </Link>
                                  </li>
