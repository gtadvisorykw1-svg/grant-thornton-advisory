        ...
        <div className="w-full px-2 sm:px-4 md:px-6 lg:px-8">
          <div className="flex h-14 sm:h-16 md:h-20 items-center justify-between gap-1 sm:gap-2">
              ...
              {Object.entries(navigationData).map(([key, data]) => (
                <button
                  ...
                  className={`flex items-center text-[15px] font-medium transition-all duration-500 relative pb-1 ${
                    activeMenu === key ? 'text-gt-purple' : 'text-gt-purple hover:text-gt-coral'
                  }`}
                >
                  ...
                  {activeMenu === key && (
                    <span className="absolute bottom-0 left-0 right-0 h-0.5 bg-gt-coral" />
                  )}
                </button>
              ))}
              ...
              {simpleNavItems.map((item) => (
                item.external ? (
                  <a
                    ...
                    className="text-[15px] font-medium text-gt-purple hover:text-gt-purple/70 transition-colors pb-1"
                  ...
                  <Link
                    key={item.label}
                    href={item.href}
                    className="text-[15px] font-medium text-gt-purple hover:text-gt-purple/70 transition-all duration-500 pb-1"
                    onClick={() => setActiveMenu(null)}
                  >
                    {item.label}
                  </Link>
            ...
            <div className="flex items-center shrink-0">
              <button
                onClick={openSearch}
                className="p-1.5 sm:p-2 text-gt-purple hover:text-gt-coral transition-all duration-500"
                aria-label="Search"
              >
              ...
              <button
                ...
                className="lg:hidden p-1.5 sm:p-2 text-gt-purple hover:text-gt-coral transition-all duration-500"
            ...
            <div className="max-w-7xl mx-auto px-16 py-12">
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-10">
                {navigationData[activeMenu as keyof typeof navigationData].columns.map((column, idx) => (
                  <div key={idx}>
                    <h3 className="text-sm font-bold text-gt-coral mb-5 uppercase tracking-wide">
                      {column.title}
                    </h3>
                    <ul className="space-y-4">
                      {column.links.map((link, linkIdx) => (
                        <li key={linkIdx}>
                          <Link
                            href={link.href}
                            className="group flex items-center text-sm text-[#333333] hover:text-gt-coral transition-all duration-500"
                            onClick={() => setActiveMenu(null)}
                          >
              ...
              {Object.entries(navigationData).map(([key, data]) => (
                <div key={key} className="border-b border-gray-200 last:border-b-0">
                  <button
                    ...
                    className="flex items-center justify-between w-full py-3 sm:py-4 text-[15px] sm:text-[16px] font-medium text-gt-purple transition-all duration-500"
                  >
                    <span>{data.label}</span>
                    <ChevronDown
                    ...
                    <div className="pb-4 sm:pb-6 pl-4 sm:pl-6">
                      {data.columns.map((column, idx) => (
                        <div key={idx} className="mb-4 sm:mb-6">
                          <h3 className="text-[11px] sm:text-xs font-bold text-gt-coral mb-2 sm:mb-3 uppercase tracking-wide">
                            {column.title}
                          </h3>
                          <ul className="space-y-2 sm:space-y-3">
                            {column.links.map((link, linkIdx) => (
                              <li key={linkIdx}>
                                <Link
                                  href={link.href}
                                  className="text-sm sm:text-base text-[#333333] hover:text-gt-coral transition-all duration-500"
                                  onClick={() => {
                                    setMobileMenuOpen(false);
              ...
              {simpleNavItems.map((item) => (
                item.external ? (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="block py-3 sm:py-4 text-[15px] sm:text-[16px] font-medium text-gt-purple border-b border-gray-200 transition-all duration-300 hover:text-gt-coral"
                  ...
                  <Link
                    key={item.label}
                    href={item.href}
                    className="block py-3 sm:py-4 text-[15px] sm:text-[16px] font-medium text-gt-purple border-b border-gray-200 transition-all duration-500 hover:text-gt-coral"
                    onClick={() => setMobileMenuOpen(false)}
                  >
                    {item.label}
                  </Link>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-8 lg:px-16 h-full flex flex-col">
            <div className="flex justify-end pt-4 sm:pt-6 md:pt-8">
              <button
                ...
                className="p-2 sm:p-3 text-gt-purple hover:text-gt-coral transition-all duration-500"
            ...
            <div className="flex-1 flex items-center justify-center pb-8 sm:pb-12 md:pb-16">
              <input
                ...
                placeholder="Search"
                autoFocus
                className="w-full max-w-5xl text-2xl sm:text-3xl md:text-4xl lg:text-6xl font-light text-gt-purple placeholder-gray-400 border-b-2 border-gray-300 focus:border-gt-coral outline-none pb-3 sm:pb-4 md:pb-5 transition-all duration-500"
              />
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-[60] bg-white overflow-auto transition-all duration-500 ease-in-out">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 lg:px-16">
            ...
            <div className="flex justify-between items-center py-4 sm:py-6 md:py-8 border-b border-gray-200">
              <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-gt-purple">Select your location</h2>
              <button
                onClick={() => setRegionOpen(false)}
                className="p-2 sm:p-3 text-gt-purple hover:text-gt-coral transition-all duration-300"
                ...
                <button
                  ...
                  className={`pb-3 sm:pb-4 md:pb-5 text-sm sm:text-base font-medium transition-all duration-500 relative ${
                    activeRegion === region.id
                      ? 'text-gt-purple'
                      : 'text-gray-500 hover:text-gt-coral'
                  }`}
                >
                  {region.label}
                  {activeRegion === region.id && (
                ...
                {countriesByRegion[activeRegion]?.map((country) => (
                  <a
                    ...
                    className={`text-sm sm:text-base font-medium transition-all duration-500 ${
                      country.name === 'Kuwait' 
                        ? 'text-gt-coral font-bold' 
                        : 'text-gt-purple hover:text-gt-coral'
                    }`}
                    onClick={() => country.href === '/' && setRegionOpen(false)}
                  >
                    {country.name}
                    {country.name === 'Kuwait' && (
                      <span className="ml-1 sm:ml-2 text-[10px] sm:text-xs bg-gt-purple text-white px-1.5 sm:px-2 py-0.5 rounded">Current</span>
                    )}
                  </a>
                ))}
              </div>
            ...
            <div className="pb-8 sm:pb-12 md:pb-16 border-t border-gray-200 pt-6 sm:pt-8 md:pt-10">
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center text-sm sm:text-base text-gt-purple hover:text-gt-coral font-medium transition-all duration-500"
              >
